namespace OpenAPI.WorldGenerator.Utils.Noise.Modules
{
    ...
    /// </summary>
    public class MultiFractalNoiseModule : FilterNoise, INoiseModule
    {
        #region Ctor/Dtor
        ...
        public float GetValue(float x, float y)
        {
            ...
            for (curOctave = 0; curOctave < _octaveCount; curOctave++)
            {
                ...
                float signal = _offset + (_source.GetValue(x, y)*_spectralWeights[curOctave]);
            ...
            if (remainder > 0.0f)
                value += remainder*_source.GetValue(x, y)*_spectralWeights[curOctave];
        ...
        public float GetValue(float x, float y, float z)
        {
            ...
            {
                // Get the coherent-noise value.
                float signal = _offset + (_source.GetValue(x, y, z)*_spectralWeights[curOctave]);
